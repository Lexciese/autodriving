...
for route in route_list:
    ...
    smooth_lats = hampel_filter(raw_lats, window_size=gnss_window_size)
    smooth_lons = hampel_filter(raw_lons, window_size=gnss_window_size)
    ...
    for i in range(0, len(file_list)):
        ...
        pd.DataFrame(euler_log).to_csv(configx.datadir+route+"/"+route+f"_ahrs_rec_hampel_w{gnss_window_size}.csv", index=False)

    #save routepoints ke yaml
    with open(configx.datadir+route+"/"+route+f"_routepoint_list_hampel_w{gnss_window_size}.yml", 'w') as c:
        yaml.dump(routes, c)
    ...
    gnss_df.to_csv(configx.datadir+route+"/"+route+f"_gnss_hampel_w{gnss_window_size}.csv", index=False)
    ...
    plt.savefig(configx.datadir+route+"/"+route+f"_routepoint_viz_hampel_w{gnss_window_size}.png", bbox_inches='tight', dpi=300)
    plt.close()
