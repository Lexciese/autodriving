...
def create_hampel_buffer(window_size=3):
    return {
        ...
        'lon_buf': deque(maxlen=window_size),
...
for route in route_list:
    ...
    for i in range(configx.gap_bearing, int(len(file_list) - (configx.n_wp * configx.wp_gap))):
        ...
        veh_curr_lat, veh_curr_lon, is_outlier = hampel_filter(
            raw_curr_lat, raw_curr_lon, stream_buffer, n_sigmas=3.0
        )

        prev_offset = -(1 + configx.gap_bearing)
        if len(stream_buffer['lat_buf']) >= abs(prev_offset):
            veh_prev_lat = stream_buffer['lat_buf'][prev_offset]
            veh_prev_lon = stream_buffer['lon_buf'][prev_offset]
        else:
