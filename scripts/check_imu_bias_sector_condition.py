...
def plot_trajectory(folder_name, all_lats, all_lons, sampled_lats, sampled_lons, file_list, sampled_indices):
    fig = go.Figure()
    ...
    sampled_labels = [
        f"#{idx+1} {file_list[idx]}<br>Lat: {all_lats[idx]:.6f}<br>Lon: {all_lons[idx]:.6f}"
    ...
    fig.update_layout(
        title=f"Filtered Trajectory & Sampled Bearing Points - {folder_name}",
    ...
    output_file = f"check_latlon_{folder_name}.html"
...
def main():
    ...
    # Arrays to store spatial points used for bearing calculation
    sampled_lats = [prev_lat]
    sampled_lons = [prev_lon]
    sampled_indices = [0]

    for i in tqdm(
        range(len(file_list)), desc="load latlon & imu with distance minima"
    ):
        ...
        curr_meta["global_position_latlon"] = [
            filtered_lats[i],
            filtered_lons[i],
        ]
        ...
        dLon_m = (
            (curr_lon - prev_lon) * 40075000 * np.cos(np.radians(curr_lat)) / 360
        )  # X
        if np.sqrt(dLat_m**2 + dLon_m**2) >= 1.0 and velocity > 2.0:
            ...
            sampled_lats.append(prev_lat)
            sampled_lons.append(prev_lon)
