...
def load_pc_from_pcd(pcd_path):
    p = PointCloud.from_path(pcd_path)
    
    # Check if intensity exists, otherwise default to 0s
    if "intensity" in p.pc_data.dtype.names:
        intensity = p.pc_data["intensity"]
    elif "reflectance" in p.pc_data.dtype.names:
        intensity = p.pc_data["reflectance"]
    else:
        intensity = np.zeros_like(p.pc_data["x"])
